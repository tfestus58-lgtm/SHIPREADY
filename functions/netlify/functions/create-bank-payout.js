 ...
 * money-moving flow.
 *
 * Always resolves to a Response. Fee-lookup failures degrade gracefully
 * (feeUnavailable: true, HTTP 200) rather than erroring, since this runs
 * live as the user types and a transient Flutterwave hiccup shouldn't flash
 * a scary error banner — the real payout call still enforces the fee
 * strictly and will fail loudly if the fee truly can't be fetched.
 */
async function buildPayoutFeePreview({ uid, amountLocal, debitCurrency, payoutCurrencyNormalized, isCrossCurrency, env }) {
  try {
    const db       = getDb(env);
    ...
    if (afterPlatformFee <= 0) {
      ...
      return new Response(JSON.stringify(basePreview), { status: 200 });
    }
    ...
    return new Response(JSON.stringify({ error: 'Could not compute fee preview right now. Please try again.' }), { status: 500 });
  }
}
    ...
    if (isFlwCurrency && payoutCurrencyNormalized !== 'NGN' && settings.flutterwaveEnabled && flwKey) {
      try {
        ...
        if (feeOrMinErr.statusCode) {
          return new Response(JSON.stringify({ error: feeOrMinErr.message }), { status: feeOrMinErr.statusCode });
        }
        ...
        return new Response(
          JSON.stringify({ error: `Could not verify current transfer fees for ${payoutCurrencyNormalized} right now. Please try again in a moment.` }),
          { status: 503 }
        );
      }
    }
